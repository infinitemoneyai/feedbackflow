"use strict";var FeedbackFlow=(()=>{var E=Object.defineProperty;var G=Object.getOwnPropertyDescriptor;var V=Object.getOwnPropertyNames;var J=Object.prototype.hasOwnProperty;var Z=(n,e)=>{for(var t in e)E(n,t,{get:e[t],enumerable:!0})},ee=(n,e,t,i)=>{if(e&&typeof e=="object"||typeof e=="function")for(let r of V(e))!J.call(n,r)&&r!==t&&E(n,r,{get:()=>e[r],enumerable:!(i=G(e,r))||i.enumerable});return n};var te=n=>ee(E({},"__esModule",{value:!0}),n);var he={};Z(he,{FeedbackFlow:()=>X,FeedbackFlowWidget:()=>g});var L={position:"bottom-right",primaryColor:"#1a1a1a",backgroundColor:"#F7F5F0",textColor:"#1a1a1a",buttonText:"Feedback",apiUrl:""};function ie(n){switch(n){case"bottom-right":return"bottom: 20px; right: 20px;";case"bottom-left":return"bottom: 20px; left: 20px;";case"top-right":return"top: 20px; right: 20px;";case"top-left":return"top: 20px; left: 20px;";default:return"bottom: 20px; right: 20px;"}}function P(n){return`
  ...
  `}var c={feedback:`<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
    ...
    <path d="M12 8h.01"></path>
      ...
      </svg>`}[e]||""}async setupCanvas(){!this.canvas||!this.capturedImage||(this.annotationCanvas=new y(this.canvas),await this.annotationCanvas.setBackgroundImage(this.capturedImage.dataUrl),this.annotationCanvas.setTool(this.currentTool))}setTool(e){this.currentTool=e,this.annotationCanvas?.setTool(e)}async retake(){this.destroy();try{this.capturedImage=await T(),this.showPreviewUI()}catch(e){l.error("Screenshot retake failed",e),this.callbacks.onCancel()}}async confirm(){if(this.annotationCanvas)try{let e=this.annotationCanvas.getDataUrl("image/png"),t=await this.annotationCanvas.getBlob("image/png"),i=await F(e,1920,.85);this.destroy(),this.callbacks.onConfirm(i)}catch(e){l.error("Failed to process screenshot",e),this.callbacks.onCancel()}}cancel(){this.destroy(),this.callbacks.onCancel()}injectStyles(){let e="ff-screenshot-styles";if(document.getElementById(e))return;let t=`
    ...
    `,i=document.createElement("style");i.id=e,i.textContent=t,document.head.appendChild(i)}destroy(){this.annotationCanvas?.destroy(),this.annotationCanvas=null,this.container?.remove(),this.container=null,this.canvas=null,this.capturedImage=null}};var B=120*1e3,ne=["video/webm;codecs=vp9,opus","video/webm;codecs=vp8,opus","video/webm;codecs=vp9","video/webm;codecs=vp8","video/webm","video/mp4"],v=class{constructor(e){this.mediaRecorder=null;this.screenStream=null;this.audioStream=null;this.combinedStream=null;this.chunks=[];this.startTime=0;this.timerInterval=null;this.mimeType="video/webm";this.callbacks=e}static isSupported(){return typeof navigator<"u"&&"mediaDevices"in navigator&&"getDisplayMedia"in navigator.mediaDevices&&typeof MediaRecorder<"u"}getSupportedMimeType(){for(let e of ne)if(MediaRecorder.isTypeSupported(e))return e;return"video/webm"}async start(){if(this.mediaRecorder?.state==="recording")throw new Error("Recording already in progress");try{this.screenStream=await navigator.mediaDevices.getDisplayMedia({video:{displaySurface:"browser",cursor:"always"},audio:!1});try{this.audioStream=await navigator.mediaDevices.getUserMedia({audio:{echoCancellation:!0,noiseSuppression:!0,autoGainControl:!0},video:!1})}catch{l.log("Microphone not available, recording without audio")}let e=[...this.screenStream.getVideoTracks(),...this.audioStream?.getAudioTracks()||[]];this.combinedStream=new MediaStream(e),this.mimeType=this.getSupportedMimeType(),this.mediaRecorder=new MediaRecorder(this.combinedStream,{mimeType:this.mimeType,videoBitsPerSecond:25e5}),this.chunks=[],this.mediaRecorder.ondataavailable=t=>{t.data.size>0&&this.chunks.push(t.data)},this.mediaRecorder.onstop=()=>{this.handleStop()},this.mediaRecorder.onerror=t=>{let i=t,r=new Error(i.error?.message||"Recording failed");this.cleanup(),this.callbacks.onError(r)},this.screenStream.getVideoTracks()[0].onended=()=>{this.mediaRecorder?.state==="recording"&&this.stop()},this.mediaRecorder.start(1e3),this.startTime=Date.now(),this.callbacks.onStart(),this.startTimer()}catch(e){throw this.cleanup(),e}}stop(){this.mediaRecorder?.state==="recording"&&this.mediaRecorder.stop(),this.stopTimer()}handleStop(){let e=Date.now()-this.startTime,t=new Blob(this.chunks,{type:this.mimeType});this.cleanup(),this.callbacks.onStop({blob:t,duration:e,mimeType:this.mimeType})}startTimer(){this.timerInterval=setInterval(()=>{let e=Date.now()-this.startTime;this.callbacks.onTimeUpdate(e),e>=B&&this.stop()},100)}stopTimer(){this.timerInterval&&(clearInterval(this.timerInterval),this.timerInterval=null)}cleanup(){this.stopTimer(),this.screenStream?.getTracks().forEach(e=>e.stop()),this.audioStream?.getTracks().forEach(e=>e.stop()),this.combinedStream?.getTracks().forEach(e=>e.stop()),this.screenStream=null,this.audioStream=null,this.combinedStream=null,this.mediaRecorder=null,this.chunks=[]}isRecording(){return this.mediaRecorder?.state==="recording"}getElapsed(){return this.startTime?Date.now()-this.startTime:0}destroy(){this.stop(),this.cleanup()}};function C(n){let e=Math.floor(n/1e3),t=Math.floor(e/60),i=e%60;return`${t.toString().padStart(2,"0")}:${i.toString().padStart(2,"0")}`}function D(){return B}var k=class{constructor(e,t){this.recorder=null;this.recordingResult=null;this.state="idle";this.indicatorElement=null;this.previewElement=null;this.videoElement=null;this.timerElement=null;this.objectUrl=null;this.config=e,this.callbacks=t,this.injectStyles()}async start(){if(!v.isSupported()){alert("Screen recording is not supported in this browser."),this.callbacks.onCancel();return}try{this.state="recording",this.showRecordingIndicator(),this.recorder=new v({onStart:()=>{l.log("Recording started")},onStop:e=>{this.handleRecordingComplete(e)},onError:e=>{l.error("Recording error",e),this.hideRecordingIndicator(),this.callbacks.onCancel()},onTimeUpdate:e=>{this.updateTimer(e)}}),await this.recorder.start()}catch(e){l.error("Failed to start recording",e),this.hideRecordingIndicator(),this.callbacks.onCancel()}}showRecordingIndicator(){this.indicatorElement=o("div",{className:"ff-recording-indicator"});let e=o("div",{className:"ff-recording-content"},[o("div",{className:"ff-recording-dot"}),o("span",{className:"ff-recording-text"},["Recording"]),o("span",{className:"ff-recording-timer"},["00:00"]),o("span",{className:"ff-recording-separator"},[" / "]),o("span",{className:"ff-recording-max"},[C(D())])]),t=o("button",{className:"ff-recording-stop",type:"button"},[d(c.stop||c.close),"Stop"]);t.addEventListener("click",()=>this.stopRecording()),this.indicatorElement.appendChild(e),this.indicatorElement.appendChild(t),document.body.appendChild(this.indicatorElement),this.timerElement=this.indicatorElement.querySelector(".ff-recording-timer")}updateTimer(e){this.timerElement&&(this.timerElement.textContent=C(e))}hideRecordingIndicator(){this.indicatorElement?.remove(),this.indicatorElement=null,this.timerElement=null}stopRecording(){this.recorder?.stop()}handleRecordingComplete(e){this.recordingResult=e,this.state="preview",this.hideRecordingIndicator(),this.showPreview()}showPreview(){if(!this.recordingResult)return;this.objectUrl=URL.createObjectURL(this.recordingResult.blob),this.previewElement=o("div",{className:"ff-recording-preview-overlay"});let e=o("div",{className:"ff-recording-preview-wrapper"}),t=o("div",{className:"ff-recording-preview-header"},[o("h3",{className:"ff-recording-preview-title"},["Preview Recording"]),this.createCloseButton()]),i=o("div",{className:"ff-recording-preview-video-container"});this.videoElement=o("video",{className:"ff-recording-preview-video"}),this.videoElement.src=this.objectUrl,this.videoElement.controls=!0,this.videoElement.playsInline=!0,i.appendChild(this.videoElement);let r=o("div",{className:"ff-recording-preview-info"},[o("span",{className:"ff-recording-info-item"},[`Duration: ${C(this.recordingResult.duration)}`]),o("span",{className:"ff-recording-info-item"},[`Size: ${(this.recordingResult.blob.size/(1024*1024)).toFixed(2)} MB`])]),s=o("div",{className:"ff-recording-preview-actions"},[this.createButton("Retake","secondary",()=>this.retake()),this.createButton("Use Recording","primary",()=>this.confirm())]);e.appendChild(t),e.appendChild(i),e.appendChild(r),e.appendChild(s),this.previewElement.appendChild(e),document.body.appendChild(this.previewElement)}createCloseButton(){let e=o("button",{className:"ff-recording-preview-close",type:"button","aria-label":"Cancel"},[d(c.close)]);return e.addEventListener("click",()=>this.cancel()),e}createButton(e,t,i){let r=o("button",{className:`ff-recording-btn ff-btn-${t}`,type:"button"},[e]);return r.addEventListener("click",i),r}hidePreview(){this.previewElement?.remove(),this.previewElement=null,this.videoElement=null,this.objectUrl&&(URL.revokeObjectURL(this.objectUrl),this.objectUrl=null)}async retake(){this.hidePreview(),this.recordingResult=null,await this.start()}confirm(){if(!this.recordingResult)return;let e=this.recordingResult;this.hidePreview(),this.callbacks.onConfirm(e)}cancel(){this.recorder?.destroy(),this.hideRecordingIndicator(),this.hidePreview(),this.recordingResult=null,this.callbacks.onCancel()}injectStyles(){let e="ff-recording-styles";if(document.getElementById(e))return;let t=`
    ...
    `,i=document.createElement("style");i.id=e,i.textContent=t,document.head.appendChild(i)}destroy(){this.recorder?.destroy(),this.hideRecordingIndicator(),this.hidePreview(),this.recordingResult=null}};var $="ff_submission_queue",se=5,ae=1e3;function le(){return`ff_${Date.now()}_${Math.random().toString(36).substring(2,9)}`}function ce(n){let e=ae*Math.pow(2,n),t=e*.2*Math.random();return Date.now()+e+t}var x=class n{constructor(e=""){this.retryTimer=null;this.isProcessing=!1;this.apiUrl=e||this.getDefaultApiUrl(),this.setupConnectivityListener(),this.scheduleRetry()}getDefaultApiUrl(){if(typeof window>"u")return"";let e=document.querySelectorAll('script[src*="widget.js"]');for(let t of Array.from(e)){let i=t.src;if(i)try{return`${new URL(i).origin}/api/widget/submit`}catch{}}return"https://feedbackflow.cc/api/widget/submit"}setupConnectivityListener(){typeof window>"u"||window.addEventListener("online",()=>{l.log("Connection restored, processing queue..."),this.processQueue()})}getQueue(){try{let e=localStorage.getItem($);return e?JSON.parse(e):[]}catch{return[]}}saveQueue(e){try{localStorage.setItem($,JSON.stringify(e))}catch(t){l.error("Failed to save queue",t)}}addToQueue(e,t,i,r){let s=this.getQueue(),a={id:le(),widgetKey:e,formData:t,screenshotDataUrl:i,recordingBlob:r?this.blobToBase64Sync(r):void 0,recordingMimeType:r?.type,timestamp:Date.now(),retryCount:0,nextRetryAt:Date.now()};return s.push(a),this.saveQueue(s),this.scheduleRetry(),a.id}blobToBase64Sync(e){return""}static blobToBase64(e){return new Promise((t,i)=>{let r=new FileReader;r.onload=()=>{let a=r.result.split(",")[1];t(a)},r.onerror=()=>i(new Error("Failed to convert blob to base64")),r.readAsDataURL(e)})}static base64ToBlob(e,t){let i=atob(e),r=new Uint8Array(i.length);for(let s=0;s<i.length;s++)r[s]=i.charCodeAt(s);return new Blob([r],{type:t})}removeFromQueue(e){let t=this.getQueue().filter(i=>i.id!==e);this.saveQueue(t)}scheduleRetry(){this.retryTimer&&clearTimeout(this.retryTimer);let e=this.getQueue();if(e.length===0)return;let t=Date.now(),i=Math.min(...e.map(s=>s.nextRetryAt)),r=Math.max(0,i-t);this.retryTimer=setTimeout(()=>{this.processQueue()},r)}async processQueue(){if(!this.isProcessing&&navigator.onLine){this.isProcessing=!0;try{let e=this.getQueue(),t=Date.now();for(let i of e)if(!(i.nextRetryAt>t)){if(i.retryCount>=se){l.warn(`Max retries exceeded for submission ${i.id}, removing from queue`),this.removeFromQueue(i.id);continue}try{let r=await this.submitToApi(i);if(r.success)l.log(`Queued submission ${i.id} succeeded, feedback ID: ${r.feedbackId}`),this.removeFromQueue(i.id),window.dispatchEvent(new CustomEvent("ff:queue-submission-success",{detail:{id:i.id,feedbackId:r.feedbackId}}));else throw new Error(r.error||"Submission failed")}catch(r){l.warn(`Queue submission ${i.id} failed, scheduling retry`,r);let s=this.getQueue(),a=s.findIndex(p=>p.id===i.id);a!==-1&&(s[a].retryCount++,s[a].nextRetryAt=ce(s[a].retryCount),this.saveQueue(s))}}}finally{this.isProcessing=!1,this.scheduleRetry()}}}async submitToApi(e){let t=new FormData;if(t.append("widgetKey",e.widgetKey),t.append("title",e.formData.title),t.append("description",e.formData.description),t.append("type",e.formData.type),t.append("metadata",JSON.stringify(e.formData.metadata)),e.formData.email&&t.append("email",e.formData.email),e.formData.name&&t.append("name",e.formData.name),e.screenshotDataUrl){let a=await(await fetch(e.screenshotDataUrl)).blob();t.append("screenshot",a,"screenshot.jpg")}if(e.recordingBlob&&e.recordingMimeType){let s=n.base64ToBlob(e.recordingBlob,e.recordingMimeType),a=e.recordingMimeType.includes("webm")?"webm":"mp4";t.append("recording",s,`recording.${a}`)}let i=await fetch(this.apiUrl,{method:"POST",body:t});if(!i.ok){let s=await i.text().catch(()=>"Unknown error");return{success:!1,error:`HTTP ${i.status}: ${s}`}}let r=await i.json();return{success:!0,feedbackId:r.feedbackId||r.id}}getQueueSize(){return this.getQueue().length}clearQueue(){this.saveQueue([]),this.retryTimer&&(clearTimeout(this.retryTimer),this.retryTimer=null)}destroy(){this.retryTimer&&(clearTimeout(this.retryTimer),this.retryTimer=null)}},M=null;function S(n){return M||(M=new x(n)),M}function H(n){let e="ff-submit-styles";if(document.getElementById(e))return;let t=`
    .ff-submit-overlay {
      position: fixed;
      top: 0;
      left: 0;
  ...
  `,i=document.createElement("style");i.id=e,i.textContent=t,document.head.appendChild(i)}function j(n,e){let t=o("div",{className:"ff-preview-thumbnail"});if(n){let i=o("img",{className:"ff-preview-img"});i.src=n.dataUrl,i.alt="Screenshot preview",t.appendChild(i),t.appendChild(o("span",{className:"ff-preview-label"},["Screenshot attached"]))}else if(e){let i=o("div",{className:"ff-preview-icon"},[d(c.video)]);t.appendChild(i);let r=Math.round(e.duration/1e3);t.appendChild(o("span",{className:"ff-preview-label"},[`Recording attached (${r}s)`]))}return t}var de=[{type:"bug",label:"Bug Report",icon:c.bug,color:"#E85D52"},{type:"feature",label:"Feature Request",icon:c.lightbulb,color:"#6B9AC4"}];function A(n,e){let t=o("div",{className:"ff-type-selector"});return de.forEach(i=>{let r=n===i.type,s=o("button",{className:`ff-type-option ${r?"ff-selected":""}`,type:"button","data-type":i.type},[o("div",{className:"ff-type-icon"},[d(i.icon)]),o("span",{className:"ff-type-label"},[i.label])]),a=s.querySelector(".ff-type-icon");a&&(a.style.color=i.color,r&&(a.style.backgroundColor=`${i.color}20`,a.style.borderColor=`${i.color}40`)),s.addEventListener("click",()=>e(i.type)),t.appendChild(s)}),t}function O(n,e){let t=o("div",{className:"ff-form-group"},[o("label",{className:"ff-form-label"},["Title"]),o("input",{className:"ff-submit-title-input ff-input",type:"text",placeholder:"Brief summary of the issue or request"})]),i=t.querySelector("input");return i.value=n,i.addEventListener("input",r=>{e(r.target.value)}),t}function W(n,e){let t=o("div",{className:"ff-form-group"},[o("label",{className:"ff-form-label"},["Description"]),o("textarea",{className:"ff-submit-description-input ff-textarea",placeholder:"Provide more details about what happened or what you'd like to see..."})]),i=t.querySelector("textarea");return i.value=n,i.addEventListener("input",r=>{e(r.target.value)}),t}function z(n,e,t,i){let r=o("div",{className:"ff-optional-section"}),s=o("div",{className:"ff-optional-header"},[o("span",{className:"ff-optional-label"},["Optional"])]);r.appendChild(s);let a=o("div",{className:"ff-form-group ff-form-group-sm"},[o("label",{className:"ff-form-label-sm"},[d(c.mail),"Email (for follow-up)"]),o("input",{className:"ff-input ff-input-sm",type:"email",placeholder:"your@email.com"})]),p=a.querySelector("input");p.value=n,p.addEventListener("input",b=>{t(b.target.value)}),r.appendChild(a);let h=o("div",{className:"ff-form-group ff-form-group-sm"},[o("label",{className:"ff-form-label-sm"},[d(c.user),"Name"]),o("input",{className:"ff-input ff-input-sm",type:"text",placeholder:"Your name"})]),f=h.querySelector("input");return f.value=e,f.addEventListener("input",b=>{i(b.target.value)}),r.appendChild(h),r}function Q(n,e,t){let i=o("div",{className:"ff-consent-section"}),r=o("input",{type:"checkbox",className:"ff-consent-checkbox",id:"ff-privacy-consent"});r.checked=e,r.addEventListener("change",a=>{t(a.target.checked)});let s=o("label",{className:"ff-consent-label",for:"ff-privacy-consent"},[r,o("span",{className:"ff-consent-text"},["I acknowledge that my feedback may include personal information and agree to the ",o("a",{href:n,target:"_blank",className:"ff-consent-link"},["privacy policy"]),"."])]);return i.appendChild(s),i}function q(){let n=o("div",{className:"ff-submit-loading"}),e=o("div",{className:"ff-spinner"},[d(c.spinner)]),t=o("p",{className:"ff-loading-text"},["Submitting your feedback..."]);return n.appendChild(e),n.appendChild(t),n}function K(n,e,t){let i=o("div",{className:"ff-submit-success"}),r=o("div",{className:"ff-success-icon"},[d(c.check)]),s=o("h3",{className:"ff-success-title"},["Feedback Submitted!"]),a=o("p",{className:"ff-success-message"},["Thank you for your feedback. We'll review it shortly."]);if(e){let f=o("div",{className:"ff-success-warning"},[o("div",{className:"ff-warning-icon"},[d(c.warning||c.info)]),o("p",{className:"ff-warning-text"},[e])]);i.appendChild(r),i.appendChild(s),i.appendChild(f)}else i.appendChild(r),i.appendChild(s),i.appendChild(a);let p=o("div",{className:"ff-success-id"},[o("span",{className:"ff-id-label"},["Reference ID: "]),o("code",{className:"ff-id-value"},[n])]),h=R("Close","primary",t);return i.appendChild(p),i.appendChild(h),i}function _(n,e,t){let i=o("div",{className:"ff-submit-error"}),r=o("div",{className:"ff-error-icon"},[d(c.close)]),s=o("h3",{className:"ff-error-title"},["Submission Failed"]),a=o("p",{className:"ff-error-message"},[n||"Something went wrong. Your feedback has been saved and will be submitted automatically when the connection is restored."]),p=o("div",{className:"ff-error-actions"},[R("Try Again","secondary",e),R("Close","primary",t)]);return i.appendChild(r),i.appendChild(s),i.appendChild(a),i.appendChild(p),i}function R(n,e,t){let i=o("button",{className:`ff-submit-btn ff-btn-${e}`,type:"button"},[n]);return i.addEventListener("click",t),i}var m=class{constructor(e,t,i,r){this.container=null;this.state="form";this.formState={title:"",description:"",type:"bug",email:"",name:"",privacyConsent:!1};this.feedbackId="";this.errorMessage="";this.warningMessage="";this.config=e,this.callbacks=t,this.screenshot=i,this.recording=r,this.offlineQueue=S(e.apiUrl)}show(){H(this.config),this.render()}render(){this.container?.remove(),this.container=o("div",{className:"ff-submit-overlay"});let e=o("div",{className:"ff-submit-wrapper"});switch(this.state){case"form":e.appendChild(this.renderForm());break;case"loading":e.appendChild(q());break;case"success":e.appendChild(K(this.feedbackId,this.warningMessage,()=>{this.callbacks.onSuccess(this.feedbackId),this.destroy()}));break;case"error":e.appendChild(_(this.errorMessage,()=>{this.state="form",this.render()},()=>{this.callbacks.onError(this.errorMessage),this.destroy()}));break}this.container.appendChild(e),document.body.appendChild(this.container),this.state==="form"&&this.container.querySelector(".ff-submit-title-input")?.focus()}renderForm(){let e=o("div",{className:"ff-submit-form"}),t=o("div",{className:"ff-submit-header"},[o("h3",{className:"ff-submit-title"},["Submit Feedback"]),this.createCloseButton()]),i=o("div",{className:"ff-submit-content"});(this.screenshot||this.recording)&&i.appendChild(j(this.screenshot,this.recording)),i.appendChild(A(this.formState.type,s=>{this.formState.type=s,this.render()})),i.appendChild(O(this.formState.title,s=>{this.formState.title=s})),i.appendChild(W(this.formState.description,s=>{this.formState.description=s})),i.appendChild(z(this.formState.email,this.formState.name,s=>{this.formState.email=s},s=>{this.formState.name=s})),this.config.privacyPolicyUrl&&i.appendChild(Q(this.config.privacyPolicyUrl,this.formState.privacyConsent,s=>{this.formState.privacyConsent=s}));let r=o("div",{className:"ff-submit-actions"},[this.createButton("Cancel","secondary",()=>this.cancel()),this.createButton("Submit Feedback","primary",()=>this.submit())]);return e.appendChild(t),e.appendChild(i),e.appendChild(r),e}createCloseButton(){let e=o("button",{className:"ff-submit-close",type:"button","aria-label":"Cancel"},[d(c.close)]);return e.addEventListener("click",()=>this.cancel()),e}createButton(e,t,i){let r=o("button",{className:`ff-submit-btn ff-btn-${t}`,type:"button"},[e]);return r.addEventListener("click",i),r}cancel(){this.destroy(),this.callbacks.onCancel()}async submit(){if(!this.formState.title.trim()){alert("Please enter a title for your feedback.");return}if(this.config.privacyPolicyUrl&&!this.formState.privacyConsent){alert("Please acknowledge the privacy policy to submit feedback.");return}this.state="loading",this.render();try{let e=await this.submitFeedback();if(e.success&&e.feedbackId)this.feedbackId=e.feedbackId,this.warningMessage=e.warning||"",this.state="success",this.render();else throw new Error(e.error||"Submission failed")}catch(e){l.error("Submission failed",e),await this.queueForRetry(),this.errorMessage=e instanceof Error?e.message:"Submission failed",this.state="error",this.render()}}async submitFeedback(){let e=this.getMetadata(),t=this.config.apiUrl||"https://feedbackflow.cc/api/widget/submit",i=new FormData;if(i.append("widgetKey",this.config.widgetKey),i.append("title",this.formState.title),i.append("description",this.formState.description),i.append("type",this.formState.type),i.append("metadata",JSON.stringify(e)),this.formState.email&&i.append("email",this.formState.email),this.formState.name&&i.append("name",this.formState.name),this.screenshot?.blob&&i.append("screenshot",this.screenshot.blob,"screenshot.jpg"),this.recording?.blob){let a=this.recording.mimeType.includes("webm")?"webm":"mp4";i.append("recording",this.recording.blob,`recording.${a}`),i.append("recordingDuration",(this.recording.duration/1e3).toString())}let r=await fetch(t,{method:"POST",body:i});if(!r.ok){let a=await r.text().catch(()=>"Unknown error");throw new Error(`HTTP ${r.status}: ${a}`)}let s=await r.json();return{success:!0,feedbackId:s.feedbackId||s.id,warning:s.warning}}async queueForRetry(){let e=this.getMetadata(),t={title:this.formState.title,description:this.formState.description,type:this.formState.type,email:this.formState.email||void 0,name:this.formState.name||void 0,metadata:e},i;this.recording?.blob&&(i=await x.blobToBase64(this.recording.blob));let r=this.offlineQueue.getQueue(),s={id:`ff_${Date.now()}_${Math.random().toString(36).substring(2,9)}`,widgetKey:this.config.widgetKey,formData:t,screenshotDataUrl:this.screenshot?.dataUrl,recordingBlob:i,recordingMimeType:this.recording?.mimeType,timestamp:Date.now(),retryCount:0,nextRetryAt:Date.now()};r.push(s),localStorage.setItem("ff_submission_queue",JSON.stringify(r)),l.log("Feedback queued for retry")}getMetadata(){return{url:window.location.href,userAgent:navigator.userAgent,timestamp:new Date().toISOString(),screenWidth:window.screen.width,screenHeight:window.screen.height}}destroy(){this.container?.remove(),this.container=null}};var g=class{constructor(e){this.root=null;this.triggerButton=null;this.modalOverlay=null;this.screenshotUI=null;this.recordUI=null;this.submitUI=null;this.capturedScreenshot=null;this.capturedRecording=null;this.config={...L,...e},this.state={isOpen:!1,isCapturing:!1,captureMode:null},this.init()}init(){N(P(this.config),"ff-widget-styles"),this.root=U(),this.createTriggerButton(),this.createModal(),this.setupEventListeners(),S(this.config.apiUrl)}createTriggerButton(){this.triggerButton=o("button",{className:"ff-trigger-button","aria-label":"Open feedback widget",type:"button"},[d(c.feedback),this.config.buttonText]),this.root?.appendChild(this.triggerButton)}createModal(){this.modalOverlay=o("div",{className:"ff-modal-overlay",role:"dialog","aria-modal":"true","aria-labelledby":"ff-modal-title"});let e=o("div",{className:"ff-modal"}),t=o("div",{className:"ff-modal-header"},[o("h2",{className:"ff-modal-title",id:"ff-modal-title"},["Share Feedback"]),this.createCloseButton()]),i=o("div",{className:"ff-modal-content"},[this.createCaptureOptions()]),r=["Powered by ",o("a",{href:"https://feedbackflow.cc",target:"_blank"},["FeedbackFlow"])];this.config.privacyPolicyUrl&&r.push(" · ",o("a",{href:this.config.privacyPolicyUrl,target:"_blank"},["Privacy Policy"]));let s=o("div",{className:"ff-modal-footer"},[o("div",{className:"ff-powered-by"},r)]);e.appendChild(t),e.appendChild(i),e.appendChild(s),this.modalOverlay.appendChild(e),this.root?.appendChild(this.modalOverlay)}createCloseButton(){return o("button",{className:"ff-close-button","aria-label":"Close feedback widget",type:"button"},[d(c.close)])}createCaptureOptions(){let e=o("div",{className:"ff-capture-options"}),t=o("button",{className:"ff-capture-option","data-capture-type":"screenshot",type:"button"},[o("div",{className:"ff-capture-icon ff-screenshot"},[d(c.camera)]),o("div",{className:"ff-capture-text"},[o("p",{className:"ff-capture-title"},["Take a Screenshot"]),o("p",{className:"ff-capture-description"},["Capture and annotate your screen"])])]),i=o("button",{className:"ff-capture-option","data-capture-type":"record",type:"button"},[o("div",{className:"ff-capture-icon ff-record"},[d(c.video)]),o("div",{className:"ff-capture-text"},[o("p",{className:"ff-capture-title"},["Record Your Screen"]),o("p",{className:"ff-capture-description"},["Record with voice narration (up to 2 min)"])])]);return e.appendChild(t),e.appendChild(i),e}setupEventListeners(){this.triggerButton?.addEventListener("click",()=>{this.open()}),this.modalOverlay?.querySelector(".ff-close-button")?.addEventListener("click",()=>{this.close()}),this.modalOverlay?.addEventListener("click",e=>{e.target===this.modalOverlay&&this.close()}),document.addEventListener("keydown",e=>{e.key==="Escape"&&this.state.isOpen&&this.close()}),this.modalOverlay?.querySelectorAll(".ff-capture-option").forEach(e=>{e.addEventListener("click",()=>{let t=e.getAttribute("data-capture-type");t&&this.startCapture(t)})})}open(){if(this.state.isOpen)return;this.state.isOpen=!0,this.modalOverlay?.classList.add("ff-visible"),this.triggerButton?.setAttribute("aria-expanded","true"),this.modalOverlay?.querySelector("button, [href], input, select, textarea")?.focus()}close(){this.state.isOpen&&(this.state.isOpen=!1,this.modalOverlay?.classList.remove("ff-visible"),this.triggerButton?.setAttribute("aria-expanded","false"),this.triggerButton?.focus())}startCapture(e){this.state.captureMode=e,this.state.isCapturing=!0,this.close();let t=new CustomEvent("ff:capture-start",{detail:{mode:e,widgetKey:this.config.widgetKey}});window.dispatchEvent(t),e==="screenshot"?this.startScreenshotCapture():e==="record"&&this.startRecordingCapture()}startScreenshotCapture(){this.screenshotUI=new w(this.config,{onConfirm:e=>{this.handleScreenshotConfirm(e)},onCancel:()=>{this.handleScreenshotCancel()}}),this.screenshotUI.start()}handleScreenshotConfirm(e){this.capturedScreenshot=e,this.state.isCapturing=!1;let t=new CustomEvent("ff:screenshot-captured",{detail:{widgetKey:this.config.widgetKey,screenshot:e}});window.dispatchEvent(t),l.log("Screenshot captured",{width:e.width,height:e.height,size:e.blob?`${(e.blob.size/1024).toFixed(2)}KB`:"unknown"}),this.screenshotUI?.destroy(),this.screenshotUI=null,this.showSubmitForm()}handleScreenshotCancel(){this.state.isCapturing=!1,this.state.captureMode=null,this.capturedScreenshot=null,this.screenshotUI?.destroy(),this.screenshotUI=null}startRecordingCapture(){this.recordUI=new k(this.config,{onConfirm:e=>{this.handleRecordingConfirm(e)},onCancel:()=>{this.handleRecordingCancel()}}),this.recordUI.start()}handleRecordingConfirm(e){this.capturedRecording=e,this.state.isCapturing=!1;let t=new CustomEvent("ff:recording-captured",{detail:{widgetKey:this.config.widgetKey,recording:{duration:e.duration,mimeType:e.mimeType,size:e.blob.size}}});window.dispatchEvent(t),l.log("Recording captured",{duration:`${(e.duration/1e3).toFixed(1)}s`,size:`${(e.blob.size/(1024*1024)).toFixed(2)}MB`,mimeType:e.mimeType}),this.recordUI?.destroy(),this.recordUI=null,this.showSubmitForm()}handleRecordingCancel(){this.state.isCapturing=!1,this.state.captureMode=null,this.capturedRecording=null,this.recordUI?.destroy(),this.recordUI=null}showSubmitForm(){this.submitUI=new m(this.config,{onSuccess:e=>{this.handleSubmissionSuccess(e)},onCancel:()=>{this.handleSubmissionCancel()},onError:e=>{this.handleSubmissionError(e)}},this.capturedScreenshot,this.capturedRecording),this.submitUI.show()}handleSubmissionSuccess(e){let t=new CustomEvent("ff:submission-success",{detail:{widgetKey:this.config.widgetKey,feedbackId:e}});window.dispatchEvent(t),l.log("Feedback submitted successfully",{feedbackId:e}),this.cleanupAfterSubmission()}handleSubmissionCancel(){l.log("Submission cancelled"),this.cleanupAfterSubmission()}handleSubmissionError(e){let t=new CustomEvent("ff:submission-error",{detail:{widgetKey:this.config.widgetKey,error:e}});window.dispatchEvent(t),l.warn("Submission error",{error:e}),this.cleanupAfterSubmission()}cleanupAfterSubmission(){this.submitUI?.destroy(),this.submitUI=null,this.capturedScreenshot=null,this.capturedRecording=null,this.state.captureMode=null}getCapturedScreenshot(){return this.capturedScreenshot}getCapturedRecording(){return this.capturedRecording}getConfig(){return{...this.config}}getState(){return{...this.state}}destroy(){this.screenshotUI?.destroy(),this.screenshotUI=null,this.recordUI?.destroy(),this.recordUI=null,this.submitUI?.destroy(),this.submitUI=null,this.root?.remove(),document.getElementById("ff-widget-styles")?.remove(),document.getElementById("ff-screenshot-styles")?.remove(),document.getElementById("ff-recording-styles")?.remove(),document.getElementById("ff-submit-styles")?.remove()}};var u=null;function pe(){let n=document.querySelectorAll("script[data-widget-key]"),e=n[n.length-1];if(!e)return l.error("No script tag with data-widget-key found"),null;let t=e.dataset.widgetKey;if(!t)return l.error("data-widget-key is required"),null;let i={widgetKey:t},r=e.dataset.position;return r&&["bottom-right","bottom-left","top-right","top-left"].includes(r)&&(i.position=r),e.dataset.primaryColor&&(i.primaryColor=e.dataset.primaryColor),e.dataset.backgroundColor&&(i.backgroundColor=e.dataset.backgroundColor),e.dataset.textColor&&(i.textColor=e.dataset.textColor),e.dataset.buttonText&&(i.buttonText=e.dataset.buttonText),e.dataset.apiUrl&&(i.apiUrl=e.dataset.apiUrl),e.dataset.privacyPolicyUrl&&(i.privacyPolicyUrl=e.dataset.privacyPolicyUrl),i}function Y(){if(u){l.warn("Widget already initialized");return}let n=pe();if(!(!n||!n.widgetKey))try{u=new g(n),l.log("Widget initialized")}catch(e){l.error("Failed to initialize widget",e)}}var X={init(n){if(u){l.warn("Widget already initialized");return}try{u=new g(n)}catch(e){l.error("Failed to initialize widget",e)}},open(){u?.open()},close(){u?.close()},getInstance(){return u},destroy(){u?.destroy(),u=null}};document.readyState==="loading"?document.addEventListener("DOMContentLoaded",Y):Y();typeof window<"u"&&(window.FeedbackFlow=X);return te(he);})();
